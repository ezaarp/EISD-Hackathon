...
export default function GradingForm({ submission }: { submission: any }) {
  ...
  const handleApprove = async () => {
    if (!confirm('Submit this grade for review?')) return;

    setIsLoading(true);
    try {
      const result = await approveGrade(submission.id, score, notes);
      if (result.requiresApproval) {
        alert('Grade submitted! Waiting for Koordinator approval.');
      } else {
  ...
  const isRecommended = submission.grade?.status === 'RECOMMENDED';
      ...
      <div className="space-y-4">
        ...
        <div>
          <label className="block text-xs text-slate-400 mb-2 uppercase font-bold">
            {isApproved || isRejected || isRecommended ? 'Graded Score' : 'Enter Score'}
          </label>
          <input
            type="number"
            value={score}
            ...
            max="100"
            step="0.5"
            disabled={isApproved || isRejected || isRecommended || isLoading}
          />
        ...
        {/* Action Buttons */}
        {!isApproved && !isRejected && !isRecommended && (
        ...
        {/* Status Display */}
        {isRecommended && (
          <div className="bg-blue-900/50 border-2 border-blue-500 p-4 text-center">
            <CheckCircle size={48} className="mx-auto text-blue-400 mb-2" />
            <p className="text-blue-400 font-bold text-lg">GRADE SUBMITTED</p>
            <p className="text-xs text-blue-300 mt-2">
              Menunggu approval dari Koordinator
            </p>
          </div>
        )}

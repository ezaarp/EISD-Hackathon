  ...
  return (
    <div className="min-h-screen bg-gray-950 flex items-center justify-center font-mono overflow-hidden relative selection:bg-red-500 selection:text-white">
      ...
      <div className="absolute inset-0 z-0 pointer-events-none" 
           style={{
             backgroundImage: 'linear-gradient(rgba(18, 16, 32, 0.9) 1px, transparent 1px), linear-gradient(90deg, rgba(18, 16, 32, 0.9) 1px, transparent 1px)',
      ...
      {/* Efek Glow Dinamis */}
      <div className={`absolute w-[700px] h-[700px] blur-[100px] rounded-full z-0 transition-colors duration-1000 ${
        mode === 'dosen' ? 'bg-red-600/30' : 
        mode === 'title' ? 'bg-blue-600/20' : 'bg-purple-600/20'
      }`}></div>
      ...
      <div className="relative z-10 flex flex-col items-center scale-90 md:scale-100 transition-transform duration-500">
        ...
        {/* HEADER / MARQUEE */}
        <div className={`w-[380px] md:w-[500px] h-32 bg-gradient-to-b rounded-t-lg border-4 border-b-0 shadow-[0_0_20px_rgba(59,130,246,0.6)] flex flex-row items-center justify-between gap-2 relative overflow-hidden px-8 transition-colors duration-500 ${
            mode === 'dosen' 
          ...
          <div className="flex flex-col z-10 text-left">
            <h1 className={`text-3xl md:text-4xl font-black italic tracking-tighter drop-shadow-[0_0_10px_rgba(255,255,255,0.8)] leading-none transition-colors duration-500 ${
                mode === 'dosen' ? 'text-red-100' : 'text-white'
            }`} style={{ fontFamily: "'Press Start 2P', cursive", textShadow: mode === 'dosen' ? "4px 4px 0px #991b1b" : "4px 4px 0px #4f46e5" }}>
              PRACTICUM
            </h1>
            <h1 className="text-4xl md:text-5xl font-black text-yellow-400 italic tracking-tighter drop-shadow-[0_0_10px_rgba(250,204,21,0.8)] leading-none mt-2" style={{ fontFamily: "'Press Start 2P', cursive", textShadow: "4px 4px 0px #b45309" }}>
              TIME
            </h1>
          </div>

          {/* LOGO AREA (RIGHT) - Recreated with SVG */}
          <div className={`relative z-10 p-2 rounded-lg border backdrop-blur-sm transition-all duration-500 flex items-center justify-center ${
              mode === 'dosen' ? 'bg-red-900/20 border-red-500/30' : 'bg-blue-900/10 border-blue-500/20'
          }`}>
             <EadLogo />
          </div>
          
          <div className={`absolute bottom-1 w-full h-1 animate-pulse transition-colors duration-500 ${
              mode === 'dosen' ? 'bg-red-500' : 
              mode === 'praktikan' ? 'bg-cyan-500' :
              mode === 'asisten' ? 'bg-pink-500' :
              'bg-indigo-500'
          }`}></div>
        </div>

        {/* MAIN BODY */}
        <div className={`w-[420px] md:w-[560px] bg-indigo-950 border-x-8 border-t-8 rounded-t-3xl rounded-b-lg p-6 pb-12 shadow-2xl relative transition-colors duration-500 ${
          ...
          <div className="bg-gray-800 p-5 rounded-xl shadow-[inset_0_0_20px_rgba(0,0,0,0.8)] border-4 border-gray-700 relative">
            ...
            {/* SCREEN (CRT) */}
            <div className={`relative w-full aspect-[4/3] bg-black rounded-lg overflow-hidden border-2 shadow-[inset_0_0_40px_rgba(0,0,0,1)] transition-colors duration-300 ${
              ...
              <div className="absolute inset-0 z-20 pointer-events-none animate-flicker opacity-10 bg-white mix-blend-overlay"></div>

              {/* CONTENT AREA */}
              <div className={`w-full h-full relative z-10 flex flex-col items-center justify-center overflow-hidden transition-all duration-300 ease-in-out ${isTransitioning ? 'scale-y-0 opacity-0 blur-md' : 'scale-y-100 opacity-100 blur-0'}`}>
                ...
                {mode === 'title' && (
                   <div className="w-full h-full flex flex-col items-center justify-center text-center p-4 cursor-pointer" onClick={handleStart}>
                      <div className="mb-10 animate-pulse-slow">
                        ...
                        <h1 className="text-yellow-400 text-2xl md:text-3xl font-bold tracking-wider leading-relaxed drop-shadow-[0_0_10px_rgba(250,204,21,0.8)]" style={{ fontFamily: "'Press Start 2P', cursive" }}>
                          EAD<br/>PRACTICUM
                        </h1>
                      </div>
                ...
                {mode === 'menu' && (
                  <div className="w-full h-full p-6 flex flex-col justify-between items-center relative overflow-hidden">
                    <div className="text-center mt-4 w-full">
                      <h2 className="text-white text-base md:text-lg font-bold tracking-widest uppercase mb-2 drop-shadow-md animate-pulse" style={{ fontFamily: "'Press Start 2P', cursive" }}>
                        SELECT CLASS
                      </h2>
                      <div className="h-0.5 w-1/2 bg-gray-600 mx-auto"></div>
                    </div>
                    ...
                    <div className="flex-1 w-full flex items-center justify-between px-4 mt-4">
                        ...
                        <div className="flex flex-col items-center animate-bounce gap-3">
                             <div className="text-cyan-400 font-bold text-xs uppercase tracking-widest drop-shadow-[0_0_5px_rgba(34,211,238,0.8)]">
                                STUDENT
                             </div>
                        ...
                        <div className="flex flex-col items-center animate-bounce gap-3" style={{ animationDelay: '0.1s' }}>
                             <div className="text-pink-400 font-bold text-xs uppercase tracking-widest drop-shadow-[0_0_5px_rgba(236,72,153,0.8)]">
                                ASSISTANT
                             </div>
                    ...
                    <button onClick={handleBackToTitle} className="w-full bg-gray-800/80 hover:bg-gray-700 text-gray-400 hover:text-white border border-gray-600 text-[10px] py-3 flex items-center justify-center gap-2 group transition-all rounded shadow-lg mb-2">
                ...
                {/* 3. LOGIN FORMS */}
                {(mode === 'praktikan' || mode === 'asisten' || mode === 'dosen') && (
                  <div className="w-full h-full flex flex-col px-6 py-4 overflow-hidden justify-center">
                    <div className={`flex items-center justify-between mb-3 border-b-2 border-dashed pb-2 shrink-0 ${
                        mode === 'dosen' ? 'border-red-800' : 'border-gray-700'
                    }`}>
                      <h2 className={`text-sm md:text-lg font-bold tracking-wider ${
                          mode === 'praktikan' ? 'text-cyan-400' : 
                          mode === 'asisten' ? 'text-pink-500' : 'text-red-500 glitch-text'
                    ...
                    <form className="space-y-3 flex-1 flex flex-col justify-center" onSubmit={(e) => e.preventDefault()}>
                      {mode === 'dosen' && (
                        ...
                        <div className="relative group">
                          <input 
                            ...
                            className={`w-full bg-gray-900/90 border-2 px-3 py-2 text-xs md:text-sm outline-none font-mono rounded transition-colors ${
                                mode === 'praktikan' ? 'border-cyan-800 focus:border-cyan-400 text-cyan-300' : 
                                mode === 'asisten' ? 'border-pink-800 focus:border-pink-400 text-pink-300' :
                      ...
                      <button className={`w-full mt-3 py-3 text-xs md:text-sm font-bold uppercase tracking-widest transition-all transform active:scale-95 shrink-0 ${
                        mode === 'praktikan' ? 'bg-cyan-700 hover:bg-cyan-600 text-white shadow-[0_0_10px_rgba(8,145,178,0.5)]' : 
                        mode === 'asisten' ? 'bg-pink-700 hover:bg-pink-600 text-white shadow-[0_0_10px_rgba(219,39,119,0.5)]' :
                        'bg-red-700 hover:bg-red-600 text-white shadow-[0_0_15px_rgba(220,38,38,0.8)] border border-red-500'
                      }`}>
                        {mode === 'dosen' ? 'OVERRIDE SYSTEM' : 'INSERT COIN (LOGIN)'}
                      </button>
                    </form>
                  </div>
                )}
          ...
          <div className="mt-8 bg-black/40 p-4 rounded-lg border-t border-white/10 relative">
            ...
            {/* Screw heads */}
            <div className="absolute top-2 left-2 w-2 h-2 rounded-full bg-gray-600 flex items-center justify-center"><div className="w-1 h-0.5 bg-gray-800 rotate-45"></div></div>
            <div className="absolute top-2 right-2 w-2 h-2 rounded-full bg-gray-600 flex items-center justify-center"><div className="w-1 h-0.5 bg-gray-800 rotate-45"></div></div>

            <div className="flex justify-between items-end px-4 md:px-6">
              
              {/* LEFT SIDE: DOSEN / GAME MASTER BUTTON */}
              <div className="flex flex-col items-center gap-2 group cursor-pointer" onClick={() => handleModeChange('dosen')}>
                ...
                <div className="text-[9px] text-red-500/70 font-bold uppercase tracking-widest text-center">
                    Dosen<br/>
                </div>
              </div>
              ...
              {mode === 'title' ? (
                ...
                <div className="flex gap-6 md:gap-8">
                  ...
                  <div className="flex flex-col items-center gap-2 group cursor-pointer" onClick={() => handleModeChange('praktikan')}>
                     <div className={`w-14 h-14 rounded-full border-b-4 transition-all active:border-b-0 active:translate-y-1 ${
                       mode === 'praktikan' 
                       ? 'bg-cyan-400 border-cyan-700 shadow-[0_0_20px_rgba(34,211,238,0.8)]' 
                       : 'bg-cyan-600 border-cyan-800 hover:bg-cyan-500'
                     }`}></div>
                  ...
                  <div className="flex flex-col items-center gap-2 group cursor-pointer" onClick={() => handleModeChange('asisten')}>
                     <div className={`w-14 h-14 rounded-full border-b-4 transition-all active:border-b-0 active:translate-y-1 ${
                       mode === 'asisten' 
                       ? 'bg-pink-500 border-pink-700 shadow-[0_0_20px_rgba(236,72,153,0.8)]' 
                       : 'bg-pink-700 border-pink-900 hover:bg-pink-600'
                     }`}></div>
          ...
          <div className="absolute bottom-6 left-6 flex gap-1">
             {[1,2,3,4].map(i => <div key={i} className="w-1 h-8 bg-black/50 rounded-full"></div>)}
          </div>
        ...
        <div className="w-[460px] md:w-[600px] h-24 bg-indigo-900 clip-path-trapezoid mt-[-10px] -z-10 flex items-center justify-center border-t-4 border-blue-800 shadow-2xl">
           <div className="flex gap-12 opacity-50">
             <div className="w-14 h-20 bg-black border-2 border-gray-700 rounded mb-4 flex items-center justify-center relative">
                <div className="w-1 h-10 bg-yellow-600/50"></div>
